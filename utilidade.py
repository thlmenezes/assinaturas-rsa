import random


#RSA
def testeMillerRabin( n:int , t = 120) -> bool:

    # Testa n menor do que 6
    if n < 6:
        return [False, False, True, True, False, True][n]
    
    # Testa se n é par
    if (n % 2 == 0):
        return False
    
    # Testa Fermat
    a = random.randint(2,n-1)
    if pow(a,int(n-1),n) != 1:
        return False

    s = 0
    q = n-1
    while (q%2 == 0):
        s += 1
        q //= 2 # NOTE: importante fazer a divisão inteira aqui
    
    for _ in range(t):
        a = random.randint(2,n-1)
        x = pow(a,int(q),n)

        if x != 1 and x != n-1:
            for _ in range(s):
                x = pow(x,2,n)
                if x == 1:
                    return False
                if x == n-1:
                    break
            if x != n-1:
                return False
    return True

def geradorPrimo(tamanho) -> int:
    tentativa_de_primo = random.getrandbits(tamanho) | (1 << tamanho-1) + 1
    while not testeMillerRabin(tentativa_de_primo,120):
        tentativa_de_primo = random.getrandbits(tamanho) | (1 << tamanho-1) + 1
    return tentativa_de_primo

def invM2(a, m) -> int:
    x = random.randrange(2,m-1)
    while (a % m) * (x % m) % m != 1:
        x = random.randrange(2,m-1)
    return x
    


#AES
Rcon = [0x01, 0x02, 0x04, 0x08, 0x10, 0x20,
        0x40, 0x80, 0x1b, 0x36, 0x6c, 0xd8, 0xab]

sbox = [
    
]

sboxInv = [
    
]

def CreateBlock(msg):
    bloco = []
    for letter in msg:
        bloco.append(letter)

    while(len(bloco) % 16 != 0):
        bloco.append(0)
    
    aux = []
    novo_bloco = []

    for j in range(0, len(bloco), 16):
        for i in range(16):
            aux.append(bloco[i+j])
        novo_bloco.append(aux)
        aux = []
        
    return novo_bloco

def DesfazBlock(bloco):
    aux = ''
    for estado in bloco:
        print(estado)
        for i in range(16):
            aux += str(estado[i])
    return aux
    

def AddRoundKey(block, key):
    for i in range(len(block)):  # xor do bloco com a chave
        block[i] = block[i] ^ key[i]

    return block


def MakeXor(c1, c2):
    c3 = []
    for i in range(len(c1)):
        c3.append(c1[i] ^ c2[i])

    return c3


def KeyExpansion(key, i):
    last_column = []
    l1, l2, l3, l4 = [], [], [], []
    for j in range(3, 16, 4):
        last_column.append(key[j])

    aux = last_column[0]
    last_column.pop(0)
    last_column.append(aux)
    last_column = SubBytes(last_column)

    for j in range(0, 4):
        if j >= 1:
            aux_col = []
            for w in range(0, 16):
                if(w % 4 == 0):
                    aux_col.append(key[w+j])

            ant_c = MakeXor(aux_col, ant_c)
            l1.append(ant_c[0])
            l2.append(ant_c[1])
            l3.append(ant_c[2])
            l4.append(ant_c[3])

        else:
            n_col = []
            n_col.extend((key[0], key[4], key[8], key[12]))
            n_col = MakeXor(last_column, n_col)
            n_col[0] = Rcon[i] ^ n_col[0]
            ant_c = n_col
            l1.append(ant_c[0])
            l2.append(ant_c[1])
            l3.append(ant_c[2])
            l4.append(ant_c[3])

    return (l1+l2+l3+l4)


def SubBytes(block):
    for i in range(len(block)):
        block[i] = sbox[block[i]]

    return block


def SubBytesInv(block):
    for i in range(len(block)):
        block[i] = sboxInv[block[i]]

    return block


def Rotate(word, n):
    return word[n:]+word[0:n]


def ShiftRows(block):
    for i in range(4):
        block[i*4:i*4+4] = Rotate(block[i*4:i*4+4], i)

    return block


def ShiftRowsInv(block):
    for i in range(4):
        block[i*4:i*4+4] = Rotate(block[i*4:i*4+4], -i)

    return block


def GaloisMult(b):
    if b & 0x80:
        b = b << 1
        b ^= 0x1B
    else:
        b = b << 1

    return b & 0xFF  # get the first 8 bits.


def mix_one_column(c):
    t = c[0] ^ c[1] ^ c[2] ^ c[3]
    u = c[0]
    c[0] ^= GaloisMult(c[0] ^ c[1]) ^ t
    c[1] ^= GaloisMult(c[1] ^ c[2]) ^ t
    c[2] ^= GaloisMult(c[2] ^ c[3]) ^ t
    c[3] ^= GaloisMult(c[3] ^ u) ^ t

    return c


def MixColumn(block):
    l1, l2, l3, l4 = [], [], [], []
    for i in range(4):
        column = []
        column.extend((block[i], block[i+4], block[i+8], block[i+12]))
        column = mix_one_column(column)
        l1.append(column[0])
        l2.append(column[1])
        l3.append(column[2])
        l4.append(column[3])

    return (l1+l2+l3+l4)


def InvMixColumn(block):
    l1, l2, l3, l4 = [], [], [], []
    for i in range(4):
        column = []
        column.extend((block[i], block[i+4], block[i+8], block[i+12]))
        l1.append(column[0])
        l2.append(column[1])
        l3.append(column[2])
        l4.append(column[3])

    for i in range(4):
        u = GaloisMult(GaloisMult(l1[i] ^ l3[i]))
        v = GaloisMult(GaloisMult(l2[i] ^ l4[i]))
        l1[i] ^= u
        l2[i] ^= v
        l3[i] ^= u
        l4[i] ^= v

    return (MixColumn(l1+l2+l3+l4))


""" def Enc(n, block, key):
    block = AddRoundKey(block, key)  # primeira iteracao
    for i in range(1, n):
        block = SubBytes(block)  # conversao do bloco utilizando a sbox
        block = ShiftRows(block)  # shift left das 'linhas'
        block = MixColumn(block)  # embaralha os elementos por coluna
        key = KeyExpansion(key, i-1)  # pega nova chave
        block = AddRoundKey(block, key)

    if(n != 0):
        block = SubBytes(block)
        block = ShiftRows(block)
        key = KeyExpansion(key, n-1)
        block = AddRoundKey(block, key)
    return block
            
def Dec(au1, block, key):
    inv_key = []
    aux = key
    inv_key.append(aux)
    for i in range(13):
        aux = KeyExpansion(aux, i)
        inv_key.append(aux)
        
    au1
    block = AddRoundKey(block, inv_key[au1])
    while(au1 > 0):
        if(au1 > 1):
            block = SubBytesInv(block)
            block = ShiftRowsInv(block)
            block = AddRoundKey(block, inv_key[au1-1])
            block = InvMixColumn(block)
        else:
            block = ShiftRowsInv(block)
            block = SubBytesInv(block)
            block = AddRoundKey(block, inv_key[0])
        au1 -= 1
    return block """